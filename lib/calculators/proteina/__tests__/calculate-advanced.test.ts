...
describe("calculateAdvancedProtein", () => {
  it("calculates protein from lean mass with goal-based g/kg", () => {
    const result = calculateAdvancedProtein(80, "hypertrophy", "strength", 4, {
      leanMass: 64,
    });

    expect(result.usesLeanMass).toBe(true);
    expect(result.idealGrams).toBe(122);
    expect(result.gramsPerKgLbm).toBeCloseTo(1.9, 1);
    expect(result.gramsPerMeal).toBe(31);
